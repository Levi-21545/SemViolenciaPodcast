...
class ProgramaPage extends StatefulWidget {
  ...
  factory ProgramaPage.fromJson(Map<String, dynamic> jsonData) {
    return ProgramaPage(
      nome: jsonData['nome'],
...
class _ProgramaState extends State<ProgramaPage> {
  Uint8List imageData = new Uint8List(500);
  Uint8List imageDataCheck = new Uint8List(500);
  String programa = '';
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => NewData(imageData, imageDataCheck),
            ),
          );
        },
...
class Edit extends StatefulWidget {
  final Episodio ep;
  Uint8List imageDataCheck = new Uint8List(500);
  Edit({required this.ep, required imageDataCheck});
  @override
  _EditState createState() => _EditState(ep, imageDataCheck);
}
...
class _EditState extends State<Edit> {
  ...
  TextEditingController cdura = new TextEditingController();
  TextEditingController carq = new TextEditingController();
  TextEditingController cthumb = new TextEditingController();
  FilePickerResult? pickedFileArq;
  FilePickerResult? pickedFileThumb;
  Uint8List? imageDataThumb;
  ...
  _EditState(this.ep, this.imageDataCheck);
  Future<String> editData() async {
    /*  print('ANTIGOS');
    print(ep.titulo);
    print(ep.duracao);
    print(ep.dia);
    ...
    var url = "http://localhost/semviolencia/editdata.php";
    var request = http.MultipartRequest('POST', Uri.parse(url));
    request.fields['titulo'] = ctitulo.text;
    request.fields['dia'] = cdia.text;
    request.fields['cdura'] = cdura.text;
    request.fields['arq'] = nmarq;
    request.fields['thumb'] = nmthumb;
    request.files.add(http.MultipartFile.fromBytes('arq', dadosArq));
    request.files.add(http.MultipartFile.fromBytes('thumb', dadosThumb));
    var res = await request.send();
    return Future.value(res.reasonPhrase);
  }

  void loadAsset() async {
    http.Response responseThumb = await http.get(
      Uri.parse(ep.thumb),
    );
    imageDataThumb = responseThumb.bodyBytes;

    http.Response responseArq = await http.get(
      Uri.parse(ep.arq),
    );
    dadosArq = responseArq.bodyBytes;
  ...
  void initState() {
    super.initState();
    ctitulo = TextEditingController(text: ep.titulo);
    cdia = TextEditingController(text: ep.dia);
    cdura = TextEditingController(text: ep.duracao);
    nmarq = ep.arq;
    nmarq = nmarq.substring(nmarq.lastIndexOf('/'));
    nmthumb = ep.thumb;
    nmthumb = nmthumb.substring(nmthumb.lastIndexOf('/'));
    loadAsset();
    dadosCheck = imageDataCheck;
    dadosThumb = imageDataThumb;
  }
    ...
    if (pickedFileArq != null) {
      try {
        setState(() {
          ...
          nmarq = pickedFileArq!.files[0].name;
          dadosArq = pickedFileArq!.files[0].bytes;
        });
      } catch (err) {
        print(err);
      }
    ...
    if (pickedFileThumb != null) {
      try {
        setState(() {
          ...
          nmthumb = pickedFileThumb!.files[0].name;
          dadosThumb = pickedFileThumb!.files[0].bytes;
        });
      } catch (err) {
        print(err);
      }
